...
\todo[inline]{
    Comment why radius decrease with mass. Can this be shown in the Newtonian limit?
}
...
This says that an increase in particle density, for example, due to compression, will result in a increase in pressure.
...
Using these general assumption, Rhoades and Ruffini found a upper limit for neutron stars of $3.2 \, M_\odot$~\cite{rhoadesMaximumMassNeutron1974}.
...
The most extreme case is the incompressible model we saw earlier, whcih break causality.
In general, a more stiff equation of state leads to a larger maximum mass.
This is intuitive; if the pressre raises faster as the density increases, then it can sustain a larger total load before collapse.


Solutions to the TOV equation are systems in hydrostatic equilibrium.
However, as a pen perfectly balancing on its edge, this does not imply stability.
The measure of stability is whether or not a system counteracts perturbation; will a disturbed system tend back towards where it started as a marble in a bowl, or will the perturbation start to grow.
We can find a necessary condition for stability by considering what happens when a star in equillibrium i perturbed slightly.
