...
The cross marks the maximum mass, $M_\mathrm{max} = 0.711 \, M_\odot$, which corresponds to a radius of $R = 9.27 \, \mathrm{km}$.
...
Unlike the other systems, it does not seem to have an upper limit for the mass.
...
To justify this, take the non-relativisitc case, $u \propto n$, in which case the assumption is equivalent to $\odv{p}/{n} \geq 0$.
...
However, as a with pen perfectly balances on its edge, equilibrium does not imply stability.
When perturbed, a stable system returns to its equilibrium position like a marble at the bottom of a bowl.
...
The equation for the eigenmodes was first obtained by Chandrasekhar~\autocite{chandrasekharDynamicalInstabilityGaseous1964}, and can be written in the form of a Sturm-Liouville equation,
%
\begin{equation}
...
One mode $\omega_n$ can change stability at a critical point on the $M-R$ curve, where
%
\begin{equation}
    \odv{M}{u_c} = 0,
\end{equation}
