...
\todo{Forlar forskjell på gravitasjons masse/ incl. bindingsenergi, flytt tolkning til etter løsningen}
...
As long as both the pressure and energy density is positive, and we are outside allways are outside the Schwarzschild radius, i.e. $r<2 G m(r)$, then $\odv{p}/{r} \leq 0$ and the pressure is strictly decreasing.
...
We already have the $rr$-component of the metric from \autoref{Definition of m(r)}, as well as an equation linking $\alpha$ directly to $p$, \autoref{alpha in terms of p and u}, which has the solution
%
\begin{equation}
    \label{solution to alpha}
    \alpha(r) = - \int_0^r \frac{\dd r'}{p(r') + u(r') } \odv{p}{r'} + \const
\end{equation}
%
The metric is thus
%
\begin{equation}
    \dd s^2
    =
    \exp(-2 \int^{p(r)} \frac{\dd p}{p + u }) \, \dd t^2
    + \left(1 - \frac{2 G m(r)}{r}\right)^{-1} \dd r^2
