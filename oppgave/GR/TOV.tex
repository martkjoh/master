...
Furthermore, as we will see later, it matches up with what we call mass in the Newtonian limit.
...
As long as both the pressure and energy density is positive, and we are outside always are outside the Schwarzschild radius, i.e., $r<2 G m(r)$, then $\odv{p}/{r} \leq 0$ and the pressure is strictly decreasing.
...
\end{equation}
%
We then impose the boundary condition $\alpha(\infty) = 0$, so that spacetime far away from the star looks flat.
