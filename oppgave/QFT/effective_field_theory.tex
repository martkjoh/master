...
\todo[inline]{Skriv om: Standard model as effective field thoery and renormalization group.}
...
Analyticity are assumptions about the poles of the S-matrix, and perturbative unitarity is that the theory should be unitary, as quantum theories should, for \emph{any order in perturbation theory}.
...
In last section, we showed that the Goldstone modes will always appear in the Lagrangian as the terms of the Mauer-Cartan form, $d_\mu$ and $e_\mu$.
...
Given an action of fields $\varphi_a$ governed by some action $S[\varphi]$, the expectation value of some functional of the fields, $F[ \varphi]$, is given by 
%
\begin{equation}
...
\end{equation}
%
Using the form of the variation, and the fact that $\eta$ is independent of $\varphi$, we may write this identity as
%
\begin{equation}
...
\end{equation}
%
Identities of these form are Ward-Takashi identities, often called just Ward-identities, and encode the symmetries of a theory.
...
These new terms in the Lagrangian, however, might break the invariance under some symmetry transformation $\varphi \rightarrow \varphi'$.
...
We can then again change variable in the path integral, which relates generating functional with different external currents, $Z[j] = Z[j']$.
...
Spinors and gauge theory, which is relevant for this example, are discussed in more depth in \autoref{chapter: chpt}.
...
Under a local $\Lie{SU}{1}$ transformation, $\psi \rightarrow e^{i\theta\alpha(x)T_\alpha} \psi$, the action changes as
%
\begin{equation}
    S \rightarrow 
    \int \dd^4 x \,
...
We then define transformations of the external fields to counteract the additional terms.
