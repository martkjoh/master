...
Stars with less mass than around $10$ times that of the sun, $M_\odot$, will towards the end of their active life shed most of their outer layers, leaving behind an inert white-hot mass only supported by the pressure from its electrons.
These remnants are called \emph{white dwarfs}.
...
Compact stars are some of the most extreme environments in our universe, and as such, they are excellent arenas for the study of exotic physics.
...
The fundamental particles of the Standard Model, as well as composite particles such as atoms or molecules, are excitations in the fields of \autoref{standard model lagrangian}.
The fundamental particles are illustrated in \autoref{fig: standard model}~\autocite{griffiths2008introduction,schwartzQuantumFieldTheory2013}
...
Processes with more interactions will contribute less to the total sum, as each interaction suppresses by $\alpha$.
...
The lightest meson, the pion, was predicted theoretically by Hideki Yukawa as the carrier of the nuclear force and later discovered by Cecil Powell \emph{et al.}~\autocite{griffiths2008introduction}.
...
As long as the system is large enough and is in thermal equilibrium, which means that the system is in a stable state where the thermodynamic variables are well-defined and without internal flows of matter or energy, then this gives an astonishingly effective and economical description of the system.
...
When increasing $\mu_B$ further, asymptotic freedom means that perturbative treatments again become available.
...
Here, quarks form Cooper pairs as electrons do in the Bardeen-Cooper-Schrieffer (BCS) theory of electromagnetic superconductors.
...
The transition from the vacuum phase to the color superconducting phase is not well understood.
...
Early lattice calculations agree with these results~\autocite{kogutQuenchedLatticeQCD2002,kogutLatticeQCDFinite2002,kogutFiniteTemperatureTransition2004,sinclairSearchingElusiveCritical2006}, 
...
In \autoref{chapter: pion stars}, we use the equations of state we found together with the TOV equation to calculate the pressure and mass profile of pion stars, as well as the mass-radius relation.
...
They are referenced in the main text when relevant.
...
The code used to derive the results of this thesis is discussed in \autoref{appendix: code}, where we also link to an online open source repository where it is available in full.
