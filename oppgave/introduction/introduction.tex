...
White dwarves, together with the even more dense \emph{neutron stars}, are collectively known as \emph{compact stars}.
...
Compact stars are some of the most extrem environments in our universe, and as such, they are excellent places for exotic physics to happen.
...
A characteristic property of fermions is the Pauli exclusion principle, which states that two identical fermions may not occupy the same quantum state.
...
While this effect supports neutron stars and white dwarves against gravity, bosons such as pions must have a repulsive interaction to exert such pressure.
...
\end{equation}
%
where $\psi_i$ are the different matter fields, the forces interactions are encoded in $D$ and $F$, and $\Phi$ is the Higgs-field~\autocite{carrollWorldEverydayExperience2013,schwartzQuantumFieldTheory2013}.
...
In the same way that we are lucky nature allows us high-energy effects and instead use an effective theory of low-energy interactions, statistical mechanics and thermodynamics allow us to describe composite systems containing a large number of degrees of freedom with only a few variables.
Instead of perfectly describing all degrees of freedom and their interaction, we consider what the average system would look like.
...
The temperature of a system, $T$, decides whether energy would flow to or from that system to a different system with a different temperature $T'$ if they are connected.
In our description of pion stars, we will assume $T = 0$, as they are much denser than they are hot and thermal effects thus can be neglected as a first approximation.
...
This is based on general relativity and describes a static ball of matter and energy in which the outward push of its internal pressure is balanced by gravity.
...
\begin{figure}[!htb]
    ...
    \caption{
        A scetch of the QCD phase diagram based on
...
Recent lattice QCD results indicate $T_{pc}$ is around $160\, \text{MeV}$.(Kilde)
...
In \autoref{chapter: math}, we summarise the mathematical prerequisites, specifically differential geometry and Lie theory.
...
In \autoref{part: chpt and the pion-condensed phase}, we apply the theory we have learned to develop and study \chpt.
We start in \autoref{chapter: chpt} by summarising QCD, which allows us to construct the effective Lagrangian of \chpt\, to leading and next-to-leading order.
...
\autoref{chapter: thermodynamics} investigate the thermodynamic properties of the condensed phases of \chpt.
...
In \autoref{Chapter: cocnlusion and discussion}, we summarise our results and discuss ways to improve them and avenues for further research.
...
\autoref{appendix: two flavor results} and \autoref{appendix: thermal field theory} are parts of the specialization project included to make the thesis more self-contained.
\autoref{appendix: two flavor results} details theory and results for the two-flavor case of \chpt, while \autoref{appendix: thermal field theory} summarises thermal field theory.
