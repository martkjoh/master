...
We see that the largest radius in our results, corresponding to  $p_c = 10^{-6} \, p_0$, is $R = 85.96 \, \text{km}$, which is in good agreement with our earlier analysis, \autoref{radius pion star nr limit}.
...
\autoref{fig: pressure and energy with EM interaction} shows the pressure and energy density, normalized to their characteristic quantities, as a function of chemical potential above the critical value, normalized to $m_pi$.
...
The new result for maximum radius, $R_\text{max} = 80.40 \, \text{km}$, is in excellent agreement with our expectation, \autoref{maximum mass pion star with em interaction}.



% \begin{figure}[!htb]
%     \centering
%     \includegraphics[width=0.8\textwidth]{../scripts/figurer/pion_star/mass_radius_pion_star_EM.pdf}
%     \caption{
%         The mass-radius relation of a pion star including electromagnetic interactions.
%         The cross indicates the maximum mass configuration, and the dot the maximum radius configuration.
%         The mass is in units of solar masses, the in kilometers.
%         }
...
We now use the next-to-leading order results for the pure pion condensate.
The mass-radius relationship of the pure pion condensate to NLO is shown in \autoref{fig: mass-radius relation compare nlo}, where it is compared to the LO result.
...
As the difference in equation of state from LO to NLO is less in the case, the difference in the mass-radius relation is too.
...
We only include the next-to-leading order result for pure pion condensation and $\pi \ell \nu_\ell$, to make the plot more orderly.
...
The key values of these maximum-mass stars are shown in \autoref{table: key values}.
...
\todo[inline]{Er det andre energigrenser vi må være under? i.e. resonanser, nukleoner?}

\begin{figure}[!htb]
    \centering
    \includegraphics[width=.8\textwidth]{../scripts/figurer/pion_star/mass_radius_all.pdf}
    \caption{
        The mass-radius relation of pion stars, including leptons to enforce charge neutrality, is compared with pion stars of only pions.
        The radius is given in kilometers and the mass in units of solar masses.
        }
...
\begin{table}[!htb]
    ...
    \begin{tabular}{l  c  c  c  c c c}
        ...
        1.732$\times10^{-2}$& \\
        $\pi  \ell \nu_\ell\,\text{NLO}$& 29.55 & 191.1 &
        0.2203  & 6.753$\times 10^{-2}$ &
        5.142$\times10^{-3}$& \\
        \hline
    \end{tabular}
\end{table}
...
Pion stars were first proposed and investigated by \citeauthor{brandtNewClassCompact2018} in \autocite{brandtNewClassCompact2018}, in which they obtained the equations of states of the systems discussed in this text, using QCD lattice methods, in which the path integral is discretized and approximated numerically.
Their results for the equation of state of the pion condensate, as well as the system with pions and electrons or muons, are shown in \autoref{fig: brandt eos} and compared with our results.
\todo[]{Use lattice values for constants to normalize lattice values}
...
\citeauthor{brandtNewClassCompact2018} do not use the same values for constants as we have used in this thesis, instead, they use~\autocite{adhikariQuarkPionAxial2021}
%
\begin{align}
    m_\pi &= 131\,\text{MeV},\\
    f_\pi &= 90.5\,\text{MeV},\\
...
\end{align}
%
In \autoref{fig: different constants compare}, we compare the results using the values of these constants as given in by the Particle Data Group and stated in \autoref{section: units}, or the constants used for lattice calculations.
