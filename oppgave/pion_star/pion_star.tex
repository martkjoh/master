...
The mass-radius relation for charge-neutral stars consisting of pions and leptons is shown in \autoref{fig: mass radius relation with leptons}, and is compared to the relation for pion of only pions.
The smaller star is due to the inclusion of the muon, while the larger is from including the electron.
We see that the two leptons affect the relationship in different ways.
The light electron does not contribute much to the energy density, and the ultrarelativistic limit of the full equation of state quickly approaches that of only the pion. 
Therefore, the maximum mass is not changed much, and the corresponding radius is only slightly larger.
However, in the non-relativistic regime, the electron results in a stiffer equation and a different polytropic index.
As expected, there is no upper limit to the radius, which only grows with lower total mass and central pressure.
...
On the other hand, the muon is much heavier and thus primarily contributes to the energy density.
...
However, this limit is only intermediate, and we should expect that as the central pressure falls even further, the radius will start to grow without bounds.
Indeed, we can notice a slight curve towards a higher radius at the lowest mass.
To demonstrate this effect further, we would need much higher precision numerical computation, and it would take us to a regime where other approximations such as zero temperature become dubious.
