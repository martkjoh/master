...
We must therefore include counter-terms from the higher-order Lagrangian.
...
Furthermore, all dependence on the unphysical parameter $\mu$ has vanished, and only the renormalized coupling constants and the mass $M$ at which they are defined remain.
 ...
 When evaluating the free energy and quantities derived from it, wee need the NLO relationship between the bare constants that appear in the Lagrangian and the physical constants.
...
We must therefore use physical values of three constants, as more would lead to an overdetermined system of equations and inconsistencies.
...
As we assume $\Delta m = 0$ and no electromagnetic interaction, the charged and neutral kaon masses are equal. \todo[]{Gjelder dette kunn på tree-level?}
...
Only as long as $\mu_I$ is smalled compared to $4 \pi f \approx 7.31 \, m_\pi$ can we expect perturbative series to converge. \todo[]{Si hvorfor det er $4 \pi f$?}
