...
integral = lambda f: (
    lambda *args: quad(f, 0, 10, args=args, epsabs=1e-5, epsrel=1e-5)[0]
    )

def f_from_df(dF):
    dF_num = lp(dF)

    df = lambda p, m, a: dF_num(p+0j, m+0j, a+0j)
...
F_0_2 = - 1/2* f**2 * (muI**2 * sin(a)**2 + 2*m**2 * cos(a) + mS**2)
...
##############
def find_alpha():
    mus = np.linspace(0, 3, 200) * m_pi
    alphas = []
    for mu in mus:
        a0 = alpha_0(mu/m_pi)[0]
        dFmu = lambda ai: dF(mu, ai)
        a1, r = newton(dFmu, a0, full_output=True)
        # print((mu/m_pi, a0, a1))
        # print(l(F_0_4.diff(a))(mu, a1))
        alphas.append(a1)
    ...
    f = np.array([F(mu, a) for mu, a in zip(mus, alphas)])
    plt.plot(mus/m_pi, f)
    plt.show()
